...
func TestDefaultSubmitter(t *testing.T) {
	...
	Convey("submitter (The default Submitter implementation)", t, func() {
		...
		Convey("submits to the configured fonero-core instance correctly", func() {
		...
		Convey("succeeds when the fonero-core responds with DUPLICATE status", func() {
			server := test.NewStaticMockServer(`{
				"status": "DUPLICATE",
				"error": null
				}`)
		...
		Convey("errors when the fonero-core url is empty", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the fonero-core url is not parseable", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "http://Not a url")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the fonero-core url is not reachable", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "http://127.0.0.1:65535")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the fonero-core returns an unparseable response", func() {
			server := test.NewStaticMockServer(`{`)
			defer server.Close()
		...
		Convey("errors when the fonero-core returns an exception response", func() {
		...
		Convey("errors when the fonero-core returns an unrecognized status", func() {
		...
		Convey("errors when the fonero-core returns an error response", func() {
