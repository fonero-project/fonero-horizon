...
func TestOperationActions_Index(t *testing.T) {
	...
	// filtered by account
	w = ht.Get("/accounts/GCC2R6IXEKBAYK3CYANYRMX5MAZXN57G2ZTKDALAZEHGCUMUDRCYVYRO/operations")
	if ht.Assert.Equal(200, w.Code) {
		ht.Assert.PageOf(3, w.Body)
	}
	...
	w = ht.Get("/transactions/GCC2R6IXEKBAYK3CYANYRMX5MAZXN57G2ZTKDALAZEHGCUMUDRCYVYRO/operations")
	if ht.Assert.Equal(200, w.Code) {
		ht.Assert.PageOf(1, w.Body)
	}
...
func TestOperationActions_Show(t *testing.T) {
	...
	if ht.Assert.Equal(200, w.Code) {
		...
		ht.Assert.Equal("GCC2R6IXEKBAYK3CYANYRMX5MAZXN57G2ZTKDALAZEHGCUMUDRCYVYRO", result.TransactionHash)
	}
