...
func initConfig() {
	...
	if viper.GetString("fonero-core-db-url") == "" {
		log.Fatal("Invalid config: fonero-core-db-url is blank.  Please specify --fonero-core-db-url on the command line or set the FONERO_CORE_DATABASE_URL environment variable.")
	}

	if viper.GetString("fonero-core-url") == "" {
		log.Fatal("Invalid config: fonero-core-url is blank.  Please specify --fonero-core-url on the command line or set the FONERO_CORE_URL environment variable.")
	}
