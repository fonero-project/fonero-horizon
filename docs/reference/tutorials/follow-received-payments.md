...
using JavaScript and `EventSource`.  We will eschew using [`js-fonero-sdk`](https://github.com/fonero-project/js-fonero-sdk), the
...
```javascript
var Keypair = require("fonero-base").Keypair;
...
The Fonero test network provides the Friendbot, a tool that developers
can use to get testnet foneros for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://horizon-testnet.fonero.org/friendbot?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Fonero network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://horizon-testnet.fonero.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/fonero-project/js-fonero-sdk).
...
var es = new EventSource('https://horizon-testnet.fonero.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://horizon-testnet.fonero.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
var keypair = FoneroBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new FoneroBase.Account(keypair.publicKey(), "713226564141056");

var asset = FoneroBase.Asset.native();
var amount = "100";
var transaction = new FoneroBase.TransactionBuilder(account)
  .addOperation(FoneroBase.Operation.payment({
    destination: FoneroBase.Keypair.random().publicKey(),
    asset: asset,
    amount: amount
  }))
  .addSigner(keypair)
